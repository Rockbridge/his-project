...
public class PatientService {
    ...
    // in de.his.patient.application.service.PatientService
    @Transactional
    public PatientResponse createPatient(CreatePatientRequest request) {
        ...
        if (request.getAddresses() != null) {
            request.getAddresses().forEach(ar -> {
                Address a = new Address();
                a.setAddressType(ar.getAddressType());
                a.setStreet(ar.getStreet());
                a.setHouseNumber(ar.getHouseNumber());
                a.setPostalCode(ar.getPostalCode());
                a.setCity(ar.getCity());
                a.setState(ar.getState());
                a.setCountry(ar.getCountry());
                patient.addAddress(a); // Lambda darf 'patient' referenzieren, weil es nicht neu zugewiesen wird
            });
        }

        Patient saved = patientRepository.save(patient); // <— neue Variable
        // Falls du weiterhin explizit speichern willst (neben Cascade):
        if (!saved.getAddresses().isEmpty()) {
            addressRepository.saveAll(saved.getAddresses());
        }

        logger.info("Created patient {} with KVNR {}", saved.getId(), saved.getKvnr());
